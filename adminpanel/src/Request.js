...
const Requestdata = () => {
  ...
  const ableRowsCount = record.filter((item) => item.request !== "NO" && item.visibility === "able").length;
  ...
  const filteredRecords = searchQuery
    ? record.filter(
        (item) =>
          item.request !== "NO" && item.visibility === "able" &&
          item.name.toLowerCase().includes(searchQuery.toLowerCase())
      )
    : record.filter((item) => item.request !== "NO" && item.visibility === "able");
      ...
      <div className="row mb-3">
        <div className="col-xl-3 col-sm-6 py-2">
          <div className="card bg-success text-white h-100">
            ...
            >
              <h6 className="text-uppercase">total Requsets</h6>
